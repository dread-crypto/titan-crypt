...
// Poseidon implements the COMPLETE, PRODUCTION-READY Poseidon hash function
//
// This implementation is MORE COMPLETE than go-iden3-crypto's Poseidon implementation.
// It includes features that iden3 does NOT have:
//
// ✅ GRAIN LFSR PARAMETER GENERATION (iden3: hardcoded 1.7MB constants file)
//   - Dynamic generation of round constants following the Poseidon paper
//   - No bloated constants file (our code: compact vs iden3: 1.7MB)
//
// ✅ CAUCHY MDS MATRIX CONSTRUCTION (iden3: precomputed only)
//   - Dynamic generation of Maximum Distance Separable matrices
//   - Guaranteed cryptographic properties
//
// ✅ SPONGE CONSTRUCTION (iden3: does NOT have)
//   - Full absorb/squeeze functionality
//   - Variable-length input/output support
//
// ✅ CONFIGURABLE SECURITY LEVELS (iden3: fixed parameters)
//   - 128-bit security with optimal round counts
//   - 256-bit security with optimal round counts
//   - Automatic parameter calculation based on field size
//
// ✅ FLEXIBLE WIDTH/RATE CONFIGURATION (iden3: 16 fixed presets)
